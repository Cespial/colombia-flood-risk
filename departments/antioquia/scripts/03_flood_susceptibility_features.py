...
def stack_all_features(
    ...
    region : ee.Geometry, optional
        Analysis region (defaults to Antioquia).
...
def generate_training_samples(
    ...
    region : ee.Geometry, optional
        Sampling region (defaults to Antioquia).
...
def run_feature_pipeline(
    ...
    if export and not samples_only:
        ...
        task = export_to_drive(
            image=feature_stack,
            description="antioquia_flood_feature_stack",
            region=region,
            scale=SAMPLE_SCALE,
        )
        tasks.append(task)
    ...
    if export:
        task = export_table_to_drive(
            collection=samples,
            description="antioquia_flood_training_samples",
            file_format="CSV",
        )
        tasks.append(task)
        ...
        task_geo = export_table_to_drive(
            collection=samples,
            description="antioquia_flood_training_samples_geo",
            file_format="GeoJSON",
        )
    ...
    if export and not samples_only:
        ...
        for group_name, img in feature_groups.items():
            task = export_to_drive(
                image=img.toFloat(),
                description=f"antioquia_features_{group_name}",
                region=region,
                scale=SAMPLE_SCALE,
            )
            tasks.append(task)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate flood susceptibility feature stack for Antioquia",
    )
    parser.add_argument(
        "--no-export",
        action="store_true",
