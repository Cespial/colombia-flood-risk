...
07_visualization.py
===================
Publication-quality figure generation for the Antioquia Flood Risk Assessment.
...
Figures:
  Fig 1  - Study area map (Antioquia, 9 subregions, rivers, elevation)
...
def fig01_study_area() -> None:
    """
    Study area map showing Antioquia department with 9 subregions,
    ...
    # Figure with two panels: (a) Colombia context, (b) Antioquia detail
    ...
    try:
        ...
        col_depts.plot(ax=ax_context, color="#f0f0f0", edgecolor="#999999",
                       linewidth=0.3)
        # Highlight Antioquia
        antioquia_proj.plot(ax=ax_context, color="#fc8d59", edgecolor="black",
                           linewidth=0.8)
    except Exception:
        antioquia_proj.plot(ax=ax_context, color="#fc8d59", edgecolor="black",
                           linewidth=0.8)
    ...
    # Antioquia border (thick)
    antioquia_proj.boundary.plot(ax=ax_main, color="black", linewidth=1.2)
    ...
    ax_main.set_title("(b) Antioquia subregions", fontsize=9, fontweight="bold")
...
def fig03_jrc_water_occurrence() -> None:
    """
    JRC Global Surface Water occurrence map for Antioquia, showing
    permanent and seasonal water bodies.
    """
    logger.info("Generating Figure 3: JRC water occurrence map...")
    set_publication_style()

    antioquia = load_antioquia_boundary("gadm").to_crs(CRS_COLOMBIA)

    # Load or synthesize JRC occurrence raster
    jrc_data = _load_or_synthesize_raster(
        "phase1_water_maps", "jrc_occurrence_antioquia.tif",
        shape=(600, 500), vmin=0, vmax=100,
    )

    fig, ax = plt.subplots(figsize=figsize_single(1.1))

    # Plot Antioquia background
    antioquia.plot(ax=ax, facecolor="#f5f5f5", edgecolor="black", linewidth=0.8)

    # Overlay JRC occurrence as imshow (placeholder extent)
    bounds = antioquia.total_bounds  # [minx, miny, maxx, maxy]
    im = ax.imshow(
        jrc_data,
    ...
    antioquia.boundary.plot(ax=ax, color="black", linewidth=1.0)
...
def fig04_flood_frequency() -> None:
    ...
    antioquia.plot(ax=ax, facecolor="#f0f0f0", edgecolor="black", linewidth=0.8)

    bounds = antioquia.total_bounds
    im = ax.imshow(
        flood_freq,
        extent=[bounds[0], bounds[2], bounds[1], bounds[3]],
        origin="upper",
...
def fig05_hand_map() -> None:
    ...
    hand_data = _load_or_synthesize_raster(
        "phase2_flood_frequency", "hand_antioquia.tif",
        shape=(600, 500), vmin=0, vmax=100,
    )
    ...
    ax.set_title("HAND Flood Susceptibility, Antioquia", fontsize=10)
    add_north_arrow(ax)
    add_scalebar(ax)
    ax.set_axis_off()
...
def fig08_susceptibility_map() -> None:
    """
    Final ensemble flood susceptibility map for Antioquia, displayed
...
def fig10_population_exposure() -> None:
    ...
    try:
        ...
        antioquia.boundary.plot(ax=ax_map, color="black", linewidth=0.8)
    except Exception:
...
def fig11_seasonal_dynamics() -> None:
    ...
    in Antioquia (bimodal pattern: MAM and SON wet seasons).
    """
