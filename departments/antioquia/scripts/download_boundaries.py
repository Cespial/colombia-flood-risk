...
def extract_antioquia_gadm() -> None:
    """
    ...
    dept_out = BOUNDARIES_DIR / "antioquia_department_boundary_GADM41.geojson"
    if not dept_out.exists() and l1_path.exists():
        with open(l1_path) as fh:
            data = json.load(fh)
        antioquia_feat = [f for f in data["features"]
                          if f["properties"].get("NAME_1") == "Antioquia"]
        if antioquia_feat:
            with open(dept_out, "w") as fh:
                json.dump({"type": "FeatureCollection", "features": antioquia_feat}, fh)
    ...
    muns_out = BOUNDARIES_DIR / "antioquia_municipalities_125_GADM41.geojson"
...
def download_geoboundaries() -> None:
    ...
    adm1_ant_out = BOUNDARIES_DIR / "antioquia_department_boundary_geoBoundaries.geojson"
    if adm1_path.exists() and not adm1_ant_out.exists():
        with open(adm1_path) as fh:
            data = json.load(fh)
        feat = [f for f in data["features"]
                if f["properties"].get("shapeName") == "Antioquia"]
        if feat:
    ...
    adm2_ant_out = BOUNDARIES_DIR / "antioquia_municipalities_geoBoundaries_simplified.geojson"
    ant_boundary_path = BOUNDARIES_DIR / "antioquia_department_boundary_GADM41.geojson"
    ...
    if (adm2_path.exists() and ant_boundary_path.exists()
        ...
        antioquia = gpd.read_file(ant_boundary_path)
        gdf_centroids = gdf.copy()
        import warnings
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            gdf_centroids.geometry = gdf.geometry.centroid
        joined = gpd.sjoin(gdf_centroids, antioquia[["geometry"]],
                           how="inner", predicate="within")
        result = gdf[gdf.index.isin(joined.index)]
        save_geojson(result, adm2_ant_out, "Antioquia municipalities (geoBoundaries simplified)")
...
def download_natural_earth() -> None:
    ...
    ant_out = BOUNDARIES_DIR / "antioquia_department_naturalearth.geojson"
    ...
    if shp_path.exists():
        ...
        antioquia = colombia[colombia["name"] == "Antioquia"]
        if not ant_out.exists():
            save_geojson(antioquia, ant_out, "Antioquia boundary (Natural Earth)")
...
def create_antioquia_subregions() -> None:
    """
    ...
    out_path = BOUNDARIES_DIR / "antioquia_9_subregions.geojson"
    if out_path.exists():
        print(f"  -> Already exists: {out_path.name}")
        return

    muns_path = BOUNDARIES_DIR / "antioquia_municipalities_125_GADM41.geojson"
    if not muns_path.exists():
        print("  WARNING: GADM municipalities file not found. Run extract_antioquia_gadm() first.")
        return
...
def download_hydrobasins() -> None:
    ...
    """
    print("\n[6] HydroBASINS – River Basins for Antioquia")
    print("    Source: https://data.hydrosheds.org")

    ant_path = BOUNDARIES_DIR / "antioquia_department_boundary_GADM41.geojson"
    if not ant_path.exists():
        print("  WARNING: Antioquia boundary not found. Run earlier steps first.")
        return

    gpd = import_geopandas()
    antioquia = gpd.read_file(ant_path)
    ...
    for level in [5, 7]:
        ...
        out_name = f"antioquia_river_basins_HydroSHEDS_L{level}.geojson"
        ...
        if shp_path.exists():
            print(f"  -> Clipping HydroBASINS L{level} to Antioquia...")
            hb = gpd.read_file(shp_path)
            clipped = gpd.sjoin(hb, antioquia[["geometry"]], how="inner",
...
def download_osm_data() -> None:
    ...
    After downloading, use osmium or ogr2ogr to clip to Antioquia.
...
def download_flood_hazard_info() -> None:
    """
    Prints information about flood hazard data sources for Antioquia.
    ...
    sources = [
        ...
        {
            "name": "Corantioquia – Cuencas (ArcGIS REST)",
            "url": "https://geografico.corantioquia.gov.co/arcgis/rest/services/Corantioquia_Geoconsulta/MapServer/6",
            "format": "ArcGIS REST / GeoJSON",
            "access": "REST API (connection issues observed)",
            "notes": "Watershed delineation (Otto Pfafstetter NSS3 level) for Corantioquia jurisdiction",
        },
        {
