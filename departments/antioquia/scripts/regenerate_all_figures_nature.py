...
========================================
Nature-quality figures for Antioquia flood risk manuscript.
...
def load_boundary():
    return gpd.read_file(BOUNDARIES_DIR / "antioquia_department_boundary_GADM41.geojson")


def load_municipalities():
    return gpd.read_file(BOUNDARIES_DIR / "antioquia_municipalities_125_GADM41.geojson")


def load_subregions():
    return gpd.read_file(BOUNDARIES_DIR / "antioquia_9_subregions.geojson")
...
def fig01_study_area():
    ...
    antioquia = load_boundary()
    subregions = load_subregions()
    munis = load_municipalities()

    antioquia_p = antioquia.to_crs(CRS_COLOMBIA)
    ...
    if has_colombia:
        ...
        antioquia_p.plot(ax=ax, color='#d4a574', edgecolor='#333333', linewidth=0.5)
    else:
        antioquia_p.plot(ax=ax, color='#d4a574', edgecolor='#333333', linewidth=0.5)
    ...
    antioquia_p.boundary.plot(ax=ax, color='#333333', linewidth=0.6)
...
def fig02_sar_water_detection():
    ...
    for ax, img, label, title in panels:
        ...
        antioquia.boundary.plot(ax=ax, color='#444444', linewidth=0.3)
