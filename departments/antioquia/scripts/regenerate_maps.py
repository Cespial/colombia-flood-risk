...
as PNG arrays, then overlays on proper Antioquia boundaries (WGS84).
...
from utils import (
    load_antioquia_boundary, load_municipalities,
...
def get_antioquia_region():
    """Get Antioquia geometry from FAO GAUL in GEE."""
    antioquia = (
        ee.FeatureCollection('FAO/GAUL/2015/level1')
        .filter(ee.Filter.eq('ADM0_NAME', 'Colombia'))
        .filter(ee.Filter.eq('ADM1_NAME', 'Antioquia'))
    )
    return antioquia.geometry().dissolve()


def get_antioquia_bbox(region):
    """Get the bounding box of Antioquia as [west, south, east, north]."""
...
def fig02_sar_water_detection():
    ...
    fig.suptitle(
        "Sentinel-1 SAR Flood Detection — Bajo Cauca, Antioquia",
...
def fig03_jrc_water_occurrence():
    """Download real JRC water occurrence and overlay on Antioquia boundary (WGS84)."""
    print("Generating Figure 3: JRC water occurrence (REAL DATA)...")
    set_publication_style()

    region = get_antioquia_region()
    bbox = get_antioquia_bbox(region)
    antioquia_gdf = load_antioquia_boundary("gadm")  # WGS84
    ...
    antioquia_gdf.boundary.plot(ax=ax, color="black", linewidth=1.2)

    # Set axis limits to Antioquia extent with small padding
    pad = 0.05
    ...
    ax.set_title("JRC Global Surface Water Occurrence, Antioquia", fontsize=10)
    add_north_arrow(ax)
