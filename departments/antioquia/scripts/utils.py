...
def get_study_area() -> ee.FeatureCollection:
    """
    Return the Antioquia department boundary from FAO GAUL Level 1.

    Uses the ``FAO/GAUL/2015/level1`` dataset and filters for
    Antioquia in Colombia.  The result is a single-feature
    ``ee.FeatureCollection`` suitable for clipping and region operations.

    Returns
    -------
    ee.FeatureCollection
        Antioquia department polygon.
    """
    antioquia = (
...
def get_municipalities() -> ee.FeatureCollection:
    """
    Return all municipalities within Antioquia from FAO GAUL Level 2.
...
def get_subregion_municipalities(subregion_name: str) -> ee.FeatureCollection:
    ...
    ----------
    subregion_name : str
        One of the 9 official Antioquia subregions defined in
...
def export_to_drive(
    ...
    folder: str = "antioquia_flood_risk",
    crs: str = "EPSG:4326",
    max_pixels: int = 1e13,
) -> ee.batch.Task:
    """
...
def get_dem() -> ee.Image:
    """
    Load the SRTM 30 m DEM clipped to Antioquia.
...
def _self_test() -> None:
    ...
    # Study area
    antioquia = get_study_area()
    n_features = safe_getinfo(antioquia.size(), label="antioquia.size()")
    _log.info("Antioquia feature count: %s", n_features)
