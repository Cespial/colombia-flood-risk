...
=========================
Sentinel-1 SAR-based surface water detection for the Bolivar department.
...
Notes for Bolivar:
  - Flat terrain favours SAR flood detection (less layover/shadow).
  - Coastal areas near Cartagena may have tidal noise (min_water_area_ha
    increased to 2.0 ha in config to mitigate).
  - Cienagas (seasonal wetlands) in the Depresion Momposina require
    multi-temporal compositing to distinguish flood from permanent water.
...
def otsu_threshold(
    ...
    region : ee.Geometry, optional
        Region for histogram computation. Defaults to Bolivar.
    scale : int
...
def detect_water_sar(
    ...
    threshold : ee.Number, optional
        Backscatter threshold (dB). Computed via Otsu if not supplied.
    band : str
        Band name to threshold.
    region : ee.Geometry, optional
        Region for Otsu computation. Defaults to Bolivar.
    min_area_ha : float
        Minimum water-body area in hectares.
...
def _add_water_band(image: ee.Image) -> ee.Image:
    """Map function: detect water and add as a band."""
    region = get_study_area_geometry()
...
def monthly_water_composite(
    ...
    -------
    ee.Image
        Binary water extent for the month.
    """
...
def export_annual_maps(
    ...
    for year in range(start_year, end_year + 1):
        ...
        if export:
            task = export_to_drive(
                ...
                description=f"bolivar_sar_flood_max_{year}",
                region=region,
                scale=cfg.S1_PARAMS["resolution"],
                folder="bolivar_flood_risk",
            )
            tasks.append(task)
    ...
    if export:
        task = export_to_drive(
            ...
            description=f"bolivar_sar_water_frequency_{start_year}_{end_year}",
            region=region,
            scale=cfg.S1_PARAMS["resolution"],
            folder="bolivar_flood_risk",
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Sentinel-1 SAR water detection for Bolivar",
    )
    parser.add_argument(
    ...
    log.info("=" * 60)
    log.info("Sentinel-1 SAR Water Detection Pipeline - Bolivar")
    log.info("=" * 60)
