...
=========================
Population and infrastructure exposure analysis for Bolivar, Colombia.
...
def _load_susceptibility_map(region: ee.Geometry) -> ee.Image:
    ...
    try:
        asset_id = "projects/ee-maestria-tesis/assets/bolivar_flood_susceptibility_ensemble"
...
def run_exposure_pipeline(export: bool = True) -> list:
    ...
    if export:
        task = export_table_to_drive(
            ...
            description="bolivar_population_exposure_by_municipality",
            file_format="CSV",
        )
        tasks.append(task)
    ...
    if export:
        task = export_table_to_drive(
            ...
            description="bolivar_area_exposure_by_landcover",
            file_format="CSV",
        )
        tasks.append(task)
    ...
    if export:
        task = export_table_to_drive(
            collection=zonal,
            description="bolivar_municipal_zonal_stats",
            file_format="CSV",
        )
        tasks.append(task)
    ...
    if export:
        task = export_table_to_drive(
            collection=ranking,
            description="bolivar_municipal_risk_ranking",
            file_format="CSV",
        )
        tasks.append(task)
    ...
    if export:
        task = export_table_to_drive(
            collection=temporal,
            description="bolivar_temporal_exposure_2015_2025",
            file_format="CSV",
        )
        tasks.append(task)
    ...
    if export:
        task = export_to_drive(
            ...
            description="bolivar_flood_risk_classified_map",
            region=region,
            scale=cfg.EXPORT_SCALE,
        )
        tasks.append(task)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Population and infrastructure exposure analysis for Bolivar",
    )
    parser.add_argument(
        "--no-export",
        action="store_true",
