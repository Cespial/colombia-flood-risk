...
======================
Climate analysis for flood context in Bolivar, Colombia.
...
# ---------------------------------------------------------------------------
# GEE helper: get Bolivar geometry
# ---------------------------------------------------------------------------

def get_bolivar_geometry() -> ee.Geometry:
    """Return the Bolivar department geometry from FAO GAUL."""
    admin = ee.FeatureCollection(ADMIN_DATASET)
    bolivar = admin.filter(ee.Filter.eq("ADM1_NAME", DEPARTMENT_NAME))
    return bolivar.geometry()
...
def compute_precipitation_trends() -> pd.DataFrame:
    ...
    the spatially averaged total precipitation over Bolivar.  Then applies
...
def compute_temperature_trends() -> pd.DataFrame:
    ...
    MODIS MOD11A2 (8-day composites) over Bolivar.
...
def compute_spi(
    ...
    """
    Compute the Standardized Precipitation Index (SPI) for Bolivar
...
def extreme_precipitation_events(
    ...
    threshold computed from the full CHIRPS record over Bolivar.
...
def precipitation_flood_correlation(
    ...
    SAR-detected flood extent over Bolivar.
...
def _plot_precipitation_trends(precip_df: pd.DataFrame) -> None:
    ...
    ax.set_title("(a) Annual precipitation trend, Bolivar (CHIRPS)")
...
def _plot_spi_timeseries(spi_df: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("SPI-3")
    ax.set_xlabel("Date")
    ax.set_title("Standardized Precipitation Index (SPI-3), Bolivar")
...
def main() -> None:
    ...
    logger.info("=" * 70)
    logger.info("CLIMATE ANALYSIS FOR FLOOD CONTEXT - BOLIVAR, COLOMBIA")
    logger.info("=" * 70)
