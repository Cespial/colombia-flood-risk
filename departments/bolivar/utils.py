...
# Expected Bolivar area in km2 (DANE official)
BOLIVAR_AREA_KM2 = 25_978.0
BOLIVAR_AREA_TOLERANCE = 0.05  # 5% tolerance for area validation
...
def load_bolivar_boundary(source: str = "gadm") -> gpd.GeoDataFrame:
    """
    Load the Bolivar department boundary polygon.
    ...
    """
    paths = {
        "gadm": BOUNDARIES_DIR / "bolivar_department_boundary_GADM41.geojson",
        "geoboundaries": BOUNDARIES_DIR / "bolivar_department_boundary_geoBoundaries.geojson",
        "naturalearth": BOUNDARIES_DIR / "bolivar_department_naturalearth.geojson",
    }
    path = paths.get(source)
    if path is None or not path.exists():
        raise FileNotFoundError(
            f"Bolivar boundary not found for source='{source}'. "
...
def load_municipalities(source: str = "gadm") -> gpd.GeoDataFrame:
    """
    Load Bolivar municipality polygons (46 municipalities).
    ...
    """
    paths = {
        "gadm": BOUNDARIES_DIR / "bolivar_municipalities_46_GADM41.geojson",
        "geoboundaries": BOUNDARIES_DIR / "bolivar_municipalities_geoBoundaries_simplified.geojson",
    }
    path = paths.get(source)
    if path is None or not path.exists():
        raise FileNotFoundError(
...
def load_subregions() -> gpd.GeoDataFrame:
    """Load the 6 official ZODES (subregions) of Bolivar."""
    path = BOUNDARIES_DIR / "bolivar_6_subregions.geojson"
    if not path.exists():
...
def load_river_basins(level: int = 5) -> gpd.GeoDataFrame:
    """
    Load HydroSHEDS river basins clipped to Bolivar.
    ...
    """
    path = BOUNDARIES_DIR / f"bolivar_river_basins_HydroSHEDS_L{level}.geojson"
    if not path.exists():
...
def validate_bolivar_area(gdf: gpd.GeoDataFrame, tolerance: float = 0.05) -> bool:
    """
    Check whether the total area of gdf is close to the expected
    Bolivar area (25,978 km2) within the given fractional tolerance.
    """
    area = compute_area_km2(gdf)
    expected = BOLIVAR_AREA_KM2
