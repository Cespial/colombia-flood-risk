...
==============================
Machine Learning flood susceptibility modelling for Cauca, Colombia.
...
def municipal_risk_stats(
    ...
    region : ee.Geometry, optional
        Clipping region (defaults to Cauca).
...
def run_ml_pipeline(
    ...
    if data_path is None:
        data_path = cfg.DATA_DIR / "satellite_exports" / "cauca_flood_training_samples.csv"
    ...
    if export_maps:
        ...
        task = export_to_drive(
            ...
            description="cauca_flood_susceptibility_ensemble",
            region=region,
            scale=cfg.EXPORT_SCALE,
        )
        tasks.append(task)
        ...
        task = export_table_to_drive(
            ...
            description="cauca_municipal_risk_stats",
            file_format="CSV",
        )
        tasks.append(task)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="ML flood susceptibility modelling for Cauca",
    )
    parser.add_argument(
        "--data-path",
        type=pathlib.Path,
