...
=========================
Population and infrastructure exposure analysis for Cauca, Colombia.
...
def _load_susceptibility_map(region: ee.Geometry) -> ee.Image:
    ...
    try:
        asset_id = "projects/ee-flood-risk-cauca/assets/cauca_flood_susceptibility_ensemble"
...
def run_exposure_pipeline(export: bool = True) -> list:
    ...
    if export:
        task = export_table_to_drive(
            ...
            description="cauca_population_exposure_by_municipality",
            file_format="CSV",
        )
        tasks.append(task)
    ...
    if export:
        task = export_table_to_drive(
            ...
            description="cauca_area_exposure_by_landcover",
            file_format="CSV",
        )
        tasks.append(task)
    ...
    if export:
        task = export_table_to_drive(
            collection=zonal,
            description="cauca_municipal_zonal_stats",
            file_format="CSV",
        )
        tasks.append(task)
    ...
    if export:
        task = export_table_to_drive(
            collection=ranking,
            description="cauca_municipal_risk_ranking",
            file_format="CSV",
        )
        tasks.append(task)
    ...
    if export:
        task = export_table_to_drive(
            collection=temporal,
            description="cauca_temporal_exposure_2015_2025",
            file_format="CSV",
        )
        tasks.append(task)
    ...
    if export:
        task = export_to_drive(
            ...
            description="cauca_flood_risk_classified_map",
            region=region,
            scale=cfg.EXPORT_SCALE,
        )
        tasks.append(task)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Population and infrastructure exposure analysis for Cauca",
    )
    parser.add_argument(
        "--no-export",
        action="store_true",
