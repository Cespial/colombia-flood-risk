...
======================
Climate analysis for flood context in Cauca, Colombia.
...
# ---------------------------------------------------------------------------
# GEE helper: get Cauca geometry
# ---------------------------------------------------------------------------

def get_cauca_geometry() -> ee.Geometry:
    """Return the Cauca department geometry from FAO GAUL."""
    admin = ee.FeatureCollection(ADMIN_DATASET)
    cauca = admin.filter(ee.Filter.eq("ADM1_NAME", DEPARTMENT_NAME))
    return cauca.geometry()
...
def compute_precipitation_trends() -> pd.DataFrame:
    ...
    the spatially averaged total precipitation over Cauca.  Then applies
...
def compute_temperature_trends() -> pd.DataFrame:
    ...
    MODIS MOD11A2 (8-day composites) over Cauca.
...
def compute_spi(
    ...
    """
    Compute the Standardized Precipitation Index (SPI) for Cauca
...
def extreme_precipitation_events(
    ...
    threshold computed from the full CHIRPS record over Cauca.
...
def precipitation_flood_correlation(
    ...
    SAR-detected flood extent over Cauca.
...
def _plot_precipitation_trends(precip_df: pd.DataFrame) -> None:
    ...
    ax.set_title("(a) Annual precipitation trend, Cauca (CHIRPS)")
...
def _plot_spi_timeseries(spi_df: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("SPI-3")
    ax.set_xlabel("Date")
    ax.set_title("Standardized Precipitation Index (SPI-3), Cauca")
...
def main() -> None:
    ...
    logger.info("=" * 70)
    logger.info("CLIMATE ANALYSIS FOR FLOOD CONTEXT - CAUCA, COLOMBIA")
    logger.info("=" * 70)
