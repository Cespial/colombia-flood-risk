...
07_visualization.py
===================
Publication-quality figure generation for the Cauca Flood Risk Assessment.
...
from utils import (
    ...
    load_cauca_boundary, load_municipalities, load_subregions,
    load_river_basins,
...
def fig01_study_area() -> None:
    ...
    # Figure with two panels: (a) Colombia context, (b) Cauca detail
    ...
    try:
        ...
        col_depts.plot(ax=ax_context, color="#f0f0f0", edgecolor="#999999",
                       linewidth=0.3)
        # Highlight Cauca
        cauca_proj.plot(ax=ax_context, color="#fc8d59", edgecolor="black",
                           linewidth=0.8)
    except Exception:
        cauca_proj.plot(ax=ax_context, color="#fc8d59", edgecolor="black",
                           linewidth=0.8)
    ...
    # Cauca border (thick)
    cauca_proj.boundary.plot(ax=ax_main, color="black", linewidth=1.2)
    ...
    ax_main.set_title("(b) Cauca subregions", fontsize=9, fontweight="bold")
...
def fig03_jrc_water_occurrence() -> None:
    ...
    cauca.boundary.plot(ax=ax, color="black", linewidth=1.0)
...
def fig04_flood_frequency() -> None:
    ...
    cauca.plot(ax=ax, facecolor="#f0f0f0", edgecolor="black", linewidth=0.8)

    bounds = cauca.total_bounds
    im = ax.imshow(
        flood_freq,
        extent=[bounds[0], bounds[2], bounds[1], bounds[3]],
        origin="upper",
...
def fig05_hand_map() -> None:
    ...
    hand_data = _load_or_synthesize_raster(
        "phase2_flood_frequency", "hand_cauca.tif",
        shape=(600, 500), vmin=0, vmax=100,
    )
...
def fig08_susceptibility_map() -> None:
    """
    Final ensemble flood susceptibility map for Cauca, displayed
...
def fig10_population_exposure() -> None:
    ...
    try:
        ...
        cauca.boundary.plot(ax=ax_map, color="black", linewidth=0.8)
    except Exception:
...
def fig11_seasonal_dynamics() -> None:
    ...
    in Cauca (bimodal pattern: MAM and SON wet seasons).
    """
...
def main() -> None:
    ...
    logger.info("=" * 70)
    logger.info("PUBLICATION FIGURE GENERATION - CAUCA FLOOD RISK ASSESSMENT")
    logger.info("=" * 70)
