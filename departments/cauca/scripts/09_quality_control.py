...
=====================
Quality control and validation for the Cauca Flood Risk Assessment.

Checks:
  1. All expected outputs exist and are non-empty
  2. Area calculations (Cauca total ~ 29,308 km2)
...
from utils import (
    ...
    compute_area_km2, validate_cauca_area,
    OUTPUTS_DIR, TABLES_DIR, FIGURES_DIR, OVERLEAF_FIGURES, OVERLEAF_TABLES,
    CAUCA_AREA_KM2, CAUCA_AREA_TOLERANCE,
...
def validate_areas() -> List[QCResult]:
    """
    Validate that area calculations for Cauca boundaries are
    consistent with the expected 29,308 km2 (DANE official area).
    ...
    for source in ["gadm", "geoboundaries", "naturalearth"]:
        try:
            boundary = load_cauca_boundary(source)
            area_km2 = compute_area_km2(boundary)
            within_tol = abs(area_km2 - CAUCA_AREA_KM2) / CAUCA_AREA_KM2 <= CAUCA_AREA_TOLERANCE
            diff_pct = (area_km2 - CAUCA_AREA_KM2) / CAUCA_AREA_KM2 * 100
            ...
            r = QCResult(
                ...
                passed=within_tol,
                message=(
                    f"Cauca area ({source}): {area_km2:,.1f} km2 "
                    f"(expected ~{CAUCA_AREA_KM2:,.0f} km2, "
                    f"diff = {diff_pct:+.2f}%)"
                ),
                details=f"Tolerance: {CAUCA_AREA_TOLERANCE*100:.0f}%",
    ...
    try:
        ...
        within_tol = abs(mun_total - CAUCA_AREA_KM2) / CAUCA_AREA_KM2 <= CAUCA_AREA_TOLERANCE
        diff_pct = (mun_total - CAUCA_AREA_KM2) / CAUCA_AREA_KM2 * 100
        ...
        r = QCResult(
            ...
            passed=within_tol,
            message=(
                f"Sum of 42 municipalities: {mun_total:,.1f} km2 "
                f"(diff = {diff_pct:+.2f}%)"
            ),
            severity="ERROR" if not within_tol else "INFO",
        )
    ...
    try:
        ...
        within_tol = abs(sub_total - CAUCA_AREA_KM2) / CAUCA_AREA_KM2 <= CAUCA_AREA_TOLERANCE
        diff_pct = (sub_total - CAUCA_AREA_KM2) / CAUCA_AREA_KM2 * 100
...
def verify_municipal_stats() -> List[QCResult]:
    ...
    - All 42 municipalities are accounted for
    ...
    if exposure_path.exists():
        ...
        if "Total Population" in exp_df.columns and "Exposed Population" in exp_df.columns:
            data_rows = exp_df[exp_df["Subregion"] != "TOTAL (Cauca)"]
...
def generate_qc_report() -> pathlib.Path:
    ...
    lines.append(f"## Cauca Flood Risk Assessment")
    lines.append("")
...
def main() -> None:
    ...
    logger.info("=" * 70)
    logger.info("QUALITY CONTROL - CAUCA FLOOD RISK ASSESSMENT")
    logger.info("=" * 70)
