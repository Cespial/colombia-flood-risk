...
======================
Downloads and processes geographic boundary data for the Cauca Flood Risk
Research Project.
...
def extract_cauca_gadm() -> None:
    ...
    # -- Cauca department boundary --
    l1_path = BOUNDARIES_DIR / "gadm41_COL_1.json"
    dept_out = BOUNDARIES_DIR / "cauca_department_boundary_GADM41.geojson"
    if not dept_out.exists() and l1_path.exists():
        with open(l1_path) as fh:
            data = json.load(fh)
        cauca_feat = [f for f in data["features"]
                          if f["properties"].get("NAME_1") == "Cauca"]
        if cauca_feat:
            with open(dept_out, "w") as fh:
                json.dump({"type": "FeatureCollection", "features": cauca_feat}, fh)
    ...
    if not muns_out.exists() and l2_path.exists():
        ...
        ant_muns = [f for f in data["features"]
                    if f["properties"].get("NAME_1") == "Cauca"]
        print(f"  -> Found {len(ant_muns)} Cauca municipalities in GADM L2")
...
def download_geoboundaries() -> None:
    ...
    # Extract Cauca from geoBoundaries ADM1
    adm1_path = BOUNDARIES_DIR / "geoBoundaries_COL_ADM1_departments.geojson"
    adm1_ant_out = BOUNDARIES_DIR / "cauca_department_boundary_geoBoundaries.geojson"
    if adm1_path.exists() and not adm1_ant_out.exists():
        with open(adm1_path) as fh:
            data = json.load(fh)
        feat = [f for f in data["features"]
                if f["properties"].get("shapeName") == "Cauca"]
        if feat:
            with open(adm1_ant_out, "w") as fh:
                json.dump({"type": "FeatureCollection", "features": feat}, fh)
            print(f"  -> Extracted Cauca from geoBoundaries ADM1")

    # Spatially extract Cauca municipalities from ADM2
    adm2_path = BOUNDARIES_DIR / "geoBoundaries_COL_ADM2_simplified.geojson"
    adm2_ant_out = BOUNDARIES_DIR / "cauca_municipalities_geoBoundaries_simplified.geojson"
    ant_boundary_path = BOUNDARIES_DIR / "cauca_department_boundary_GADM41.geojson"

    if (adm2_path.exists() and ant_boundary_path.exists()
            and not adm2_ant_out.exists()):
        print("  -> Spatially extracting Cauca municipalities from geoBoundaries ADM2...")
        gdf = gpd.read_file(adm2_path)
        cauca = gpd.read_file(ant_boundary_path)
        gdf_centroids = gdf.copy()
        import warnings
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            gdf_centroids.geometry = gdf.geometry.centroid
        joined = gpd.sjoin(gdf_centroids, cauca[["geometry"]],
                           how="inner", predicate="within")
        result = gdf[gdf.index.isin(joined.index)]
        save_geojson(result, adm2_ant_out, "Cauca municipalities (geoBoundaries simplified)")


def download_natural_earth() -> None:
    """
    Download Natural Earth 1:10m admin-1 shapefile and extract
    Cauca + all Colombia departments.
    """
    ...
    ant_out = BOUNDARIES_DIR / "cauca_department_naturalearth.geojson"
    ...
    if shp_path.exists():
        ...
        cauca = colombia[colombia["name"] == "Cauca"]
        if not ant_out.exists():
            save_geojson(cauca, ant_out, "Cauca boundary (Natural Earth)")


# ---------------------------------------------------------------------------
# Section 2 – Cauca Subregions
# ---------------------------------------------------------------------------
...
def download_hydrobasins() -> None:
    ...
    """
    print("\n[6] HydroBASINS – River Basins for Cauca")
    print("    Source: https://data.hydrosheds.org")

    ant_path = BOUNDARIES_DIR / "cauca_department_boundary_GADM41.geojson"
    if not ant_path.exists():
        print("  WARNING: Cauca boundary not found. Run earlier steps first.")
        return

    gpd = import_geopandas()
    cauca = gpd.read_file(ant_path)
    ...
    for level in [5, 7]:
        ...
        out_name = f"cauca_river_basins_HydroSHEDS_L{level}.geojson"
        ...
        if shp_path.exists():
            print(f"  -> Clipping HydroBASINS L{level} to Cauca...")
            hb = gpd.read_file(shp_path)
            clipped = gpd.sjoin(hb, cauca[["geometry"]], how="inner",
                                predicate="intersects")
            clipped = clipped.drop(columns=["index_right"], errors="ignore")
            save_geojson(clipped, out_path,
                         f"Cauca river basins HydroSHEDS L{level}")
...
def download_osm_data() -> None:
    ...
    After downloading, use osmium or ogr2ogr to clip to Cauca.

    For a smaller targeted download, use the BBBike custom extract service:
      https://extract.bbbike.org/
    Define the Cauca bounding box: W=-77.15, S=5.41, E=-73.87, N=8.89
...
def download_flood_hazard_info() -> None:
    """
    Prints information about flood hazard data sources for Cauca.
    ...
    sources = [
        ...
        },
        {
            ...
            "notes": "Search for 'inundaciones', 'eventos', 'emergencias Cauca'",
        },
        {
...
def main() -> None:
    print("=" * 70)
    print("Cauca Flood Risk Research – Geographic Boundary Downloader")
    print("=" * 70)
