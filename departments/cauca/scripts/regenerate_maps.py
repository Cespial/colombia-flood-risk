...
as PNG arrays, then overlays on proper Cauca boundaries (WGS84).
...
from utils import (
    load_cauca_boundary, load_municipalities,
...
import ee
try:
    ee.Initialize(project='ee-flood-risk-cauca')
except Exception:
    ee.Authenticate()
    ee.Initialize(project='ee-flood-risk-cauca')
...
def get_cauca_region():
    """Get Cauca geometry from FAO GAUL in GEE."""
    cauca = (
        ee.FeatureCollection('FAO/GAUL/2015/level1')
        .filter(ee.Filter.eq('ADM0_NAME', 'Colombia'))
        .filter(ee.Filter.eq('ADM1_NAME', 'Cauca'))
    )
    return cauca.geometry().dissolve()


def get_cauca_bbox(region):
    """Get the bounding box of Cauca as [west, south, east, north]."""
...
def fig03_jrc_water_occurrence():
    """Download real JRC water occurrence and overlay on Cauca boundary (WGS84)."""
    print("Generating Figure 3: JRC water occurrence (REAL DATA)...")
    set_publication_style()

    region = get_cauca_region()
    bbox = get_cauca_bbox(region)
    cauca_gdf = load_cauca_boundary("gadm")  # WGS84
    ...
    cauca_gdf.boundary.plot(ax=ax, color="black", linewidth=1.2)

    # Set axis limits to Cauca extent with small padding
    pad = 0.05
