...
utils.py
========
Shared utility functions for the Cauca Flood Risk Assessment project.
...
Author : Cauca Flood Risk Research Project
Date   : 2026-02-26
"""
...
def get_study_area() -> ee.FeatureCollection:
    """
    Return the Cauca department boundary from FAO GAUL Level 1.

    Uses the ``FAO/GAUL/2015/level1`` dataset and filters for
    Cauca in Colombia.  The result is a single-feature
    ``ee.FeatureCollection`` suitable for clipping and region operations.

    Returns
    -------
    ee.FeatureCollection
        Cauca department polygon.
    """
    cauca = (
...
def get_study_area_geometry() -> ee.Geometry:
    """
    Return the dissolved geometry of Cauca as a single ``ee.Geometry``.
...
def get_municipalities() -> ee.FeatureCollection:
    """
    Return all municipalities within Cauca from FAO GAUL Level 2.
...
def get_subregion_municipalities(subregion_name: str) -> ee.FeatureCollection:
    """
    Filter municipalities belonging to a specific Cauca subregion.
...
def get_dem() -> ee.Image:
    """
    Load the SRTM 30 m DEM clipped to Cauca.
...
def _self_test() -> None:
    ...
    # Study area
    cauca = get_study_area()
    n_features = safe_getinfo(cauca.size(), label="cauca.size()")
    _log.info("Cauca feature count: %s", n_features)
