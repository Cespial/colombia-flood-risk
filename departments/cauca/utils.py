...
def load_cauca_boundary(source: str = "gadm") -> gpd.GeoDataFrame:
    """
    Load the Cauca department boundary polygon.
    ...
    """
    paths = {
        "gadm": BOUNDARIES_DIR / "cauca_department_boundary_GADM41.geojson",
        "geoboundaries": BOUNDARIES_DIR / "cauca_department_boundary_geoBoundaries.geojson",
        "naturalearth": BOUNDARIES_DIR / "cauca_department_naturalearth.geojson",
    }
    path = paths.get(source)
    if path is None or not path.exists():
        raise FileNotFoundError(
            f"Cauca boundary not found for source='{source}'. "
...
def load_river_basins(level: int = 5) -> gpd.GeoDataFrame:
    """
    Load HydroSHEDS river basins clipped to Cauca.
    ...
    """
    path = BOUNDARIES_DIR / f"cauca_river_basins_HydroSHEDS_L{level}.geojson"
    if not path.exists():
