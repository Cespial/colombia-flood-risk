...
=========================
Sentinel-1 SAR-based surface water detection for the choco department.
...
def otsu_threshold(
    ...
    region : ee.Geometry, optional
        Region for histogram computation. Defaults to choco.
    scale : int
...
def detect_water_sar(
    ...
    band : str
        Band name to threshold.
    region : ee.Geometry, optional
        Region for Otsu computation. Defaults to choco.
    min_area_ha : float
...
def export_annual_maps(
    ...
    for year in range(start_year, end_year + 1):
        ...
        if export:
            task = export_to_drive(
                ...
                description=f"choco_sar_flood_max_{year}",
                region=region,
                scale=cfg.S1_PARAMS["resolution"],
                folder="choco_flood_risk",
            )
            tasks.append(task)
    ...
    if export:
        task = export_to_drive(
            ...
            description=f"choco_sar_water_frequency_{start_year}_{end_year}",
            region=region,
            scale=cfg.S1_PARAMS["resolution"],
            folder="choco_flood_risk",
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Sentinel-1 SAR water detection for choco",
    )
    parser.add_argument(
        "--year",
        type=int,
