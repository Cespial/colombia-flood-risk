...
========================
Analysis of the JRC Global Surface Water (GSW) v1.4 dataset for choco.
...
def seasonal_dynamics(region: ee.Geometry) -> Dict[str, ee.Image]:
    """
    Compute seasonal water extent differences for choco.
...
def run_jrc_analysis(export: bool = True) -> list:
    ...
    if export:
        task = export_to_drive(
            ...
            description="choco_jrc_flood_frequency",
            region=region,
            scale=cfg.EXPORT_SCALE,
        )
        tasks.append(task)
    ...
    if export:
        ...
        task = export_to_drive(
            ...
            description="choco_jrc_seasonal_dynamics",
            region=region,
            scale=cfg.EXPORT_SCALE,
        )
        tasks.append(task)
    ...
    if export:
        task = export_to_drive(
            ...
            description="choco_jrc_water_trends",
            region=region,
            scale=cfg.EXPORT_SCALE,
        )
        tasks.append(task)
    ...
    if export:
        task = export_table_to_drive(
            ...
            description="choco_jrc_municipal_occurrence_stats",
            file_format="CSV",
        )
        tasks.append(task)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="JRC Global Surface Water analysis for choco",
    )
    parser.add_argument(
        "--no-export",
        action="store_true",
