...
======================
Climate analysis for flood context in choco, Colombia.
...
# ---------------------------------------------------------------------------
# GEE helper: get choco geometry
# ---------------------------------------------------------------------------

def get_choco_geometry() -> ee.Geometry:
    """Return the choco department geometry from FAO GAUL."""
    admin = ee.FeatureCollection(ADMIN_DATASET)
    choco = admin.filter(ee.Filter.eq("ADM1_NAME", DEPARTMENT_NAME))
    return choco.geometry()
...
def compute_precipitation_trends() -> pd.DataFrame:
    ...
    the spatially averaged total precipitation over choco.  Then applies
...
def compute_temperature_trends() -> pd.DataFrame:
    ...
    MODIS MOD11A2 (8-day composites) over choco.
...
def compute_spi(
    ...
    """
    Compute the Standardized Precipitation Index (SPI) for choco
...
def extreme_precipitation_events(
    ...
    threshold computed from the full CHIRPS record over choco.
...
def precipitation_flood_correlation(
    ...
    SAR-detected flood extent over choco.
...
def _plot_precipitation_trends(precip_df: pd.DataFrame) -> None:
    ...
    ax.set_title("(a) Annual precipitation trend, choco (CHIRPS)")
...
def _plot_spi_timeseries(spi_df: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("SPI-3")
    ax.set_xlabel("Date")
    ax.set_title("Standardized Precipitation Index (SPI-3), choco")
...
def main() -> None:
    ...
    logger.info("=" * 70)
    logger.info("CLIMATE ANALYSIS FOR FLOOD CONTEXT - choco, COLOMBIA")
    logger.info("=" * 70)
