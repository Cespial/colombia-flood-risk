...
07_visualization.py
===================
Publication-quality figure generation for the choco Flood Risk Assessment.
...
Figures:
  Fig 1  - Study area map (choco, 9 subregions, rivers, elevation)
...
from utils import (
    ...
    load_department_boundary, load_municipalities, load_subregions,
    load_river_basins,
...
def fig01_study_area() -> None:
    """
    Study area map showing choco department with 9 subregions,
    ...
    # Figure with two panels: (a) Colombia context, (b) choco detail
    ...
    try:
        ...
        col_depts.plot(ax=ax_context, color="#f0f0f0", edgecolor="#999999",
                       linewidth=0.3)
        # Highlight choco
        choco_proj.plot(ax=ax_context, color="#fc8d59", edgecolor="black",
                           linewidth=0.8)
    except Exception:
        choco_proj.plot(ax=ax_context, color="#fc8d59", edgecolor="black",
                           linewidth=0.8)
    ...
    # choco border (thick)
    choco_proj.boundary.plot(ax=ax_main, color="black", linewidth=1.2)
    ...
    ax_main.set_title("(b) choco subregions", fontsize=9, fontweight="bold")
...
def fig03_jrc_water_occurrence() -> None:
    ...
    choco.boundary.plot(ax=ax, color="black", linewidth=1.0)
...
def fig04_flood_frequency() -> None:
    ...
    choco.plot(ax=ax, facecolor="#f0f0f0", edgecolor="black", linewidth=0.8)

    bounds = choco.total_bounds
    im = ax.imshow(
        flood_freq,
        extent=[bounds[0], bounds[2], bounds[1], bounds[3]],
        origin="upper",
...
def fig05_hand_map() -> None:
    ...
    hand_data = _load_or_synthesize_raster(
        "phase2_flood_frequency", "hand_choco.tif",
        shape=(600, 500), vmin=0, vmax=100,
    )
    ...
    ax.set_title("HAND Flood Susceptibility, choco", fontsize=10)
    add_north_arrow(ax)
    add_scalebar(ax)
    ax.set_axis_off()
...
def fig08_susceptibility_map() -> None:
    """
    Final ensemble flood susceptibility map for choco, displayed
...
def fig10_population_exposure() -> None:
    ...
    try:
        ...
        choco.boundary.plot(ax=ax_map, color="black", linewidth=0.8)
    except Exception:
...
def fig11_seasonal_dynamics() -> None:
    ...
    in choco (bimodal pattern: MAM and SON wet seasons).
    """
...
def main() -> None:
    ...
    logger.info("=" * 70)
    logger.info("PUBLICATION FIGURE GENERATION - choco FLOOD RISK ASSESSMENT")
    logger.info("=" * 70)
