...
=====================
Quality control and validation for the choco Flood Risk Assessment.

Checks:
  1. All expected outputs exist and are non-empty
  2. Area calculations (choco total ~ 63,612 km2)
...
from utils import (
    ...
    compute_area_km2, validate_department_area,
    OUTPUTS_DIR, TABLES_DIR, FIGURES_DIR, OVERLEAF_FIGURES, OVERLEAF_TABLES,
    choco_AREA_KM2, choco_AREA_TOLERANCE,
...
def validate_areas() -> List[QCResult]:
    """
    Validate that area calculations for choco boundaries are
    ...
    for source in ["gadm", "geoboundaries", "naturalearth"]:
        try:
            boundary = load_department_boundary(source)
            area_km2 = compute_area_km2(boundary)
            within_tol = abs(area_km2 - choco_AREA_KM2) / choco_AREA_KM2 <= choco_AREA_TOLERANCE
            diff_pct = (area_km2 - choco_AREA_KM2) / choco_AREA_KM2 * 100
            ...
            r = QCResult(
                ...
                passed=within_tol,
                message=(
                    f"choco area ({source}): {area_km2:,.1f} km2 "
                    f"(expected ~{choco_AREA_KM2:,.0f} km2, "
                    f"diff = {diff_pct:+.2f}%)"
                ),
                details=f"Tolerance: {choco_AREA_TOLERANCE*100:.0f}%",
    ...
    try:
        ...
        within_tol = abs(mun_total - choco_AREA_KM2) / choco_AREA_KM2 <= choco_AREA_TOLERANCE
        diff_pct = (mun_total - choco_AREA_KM2) / choco_AREA_KM2 * 100
    ...
    try:
        ...
        within_tol = abs(sub_total - choco_AREA_KM2) / choco_AREA_KM2 <= choco_AREA_TOLERANCE
        diff_pct = (sub_total - choco_AREA_KM2) / choco_AREA_KM2 * 100
...
def verify_municipal_stats() -> List[QCResult]:
    ...
    if exposure_path.exists():
        ...
        if "Total Population" in exp_df.columns and "Exposed Population" in exp_df.columns:
            data_rows = exp_df[exp_df["Subregion"] != "TOTAL (choco)"]
...
def generate_qc_report() -> pathlib.Path:
    ...
    lines.append(f"## choco Flood Risk Assessment")
    lines.append("")
...
def main() -> None:
    ...
    logger.info("=" * 70)
    logger.info("QUALITY CONTROL - choco FLOOD RISK ASSESSMENT")
    logger.info("=" * 70)
