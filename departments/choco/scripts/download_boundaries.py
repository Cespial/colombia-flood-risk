...
def extract_choco_gadm() -> None:
    """
    Extract choco-specific GeoJSON files from GADM Colombia data:
      - Department boundary (level 1)
      - All 125 municipalities (level 2)
    """
    gpd = import_geopandas()
    print("\n[2] Extracting choco boundaries from GADM data")

    # -- choco department boundary --
    l1_path = BOUNDARIES_DIR / "gadm41_COL_1.json"
    dept_out = BOUNDARIES_DIR / "choco_department_boundary_GADM41.geojson"
    if not dept_out.exists() and l1_path.exists():
        with open(l1_path) as fh:
            data = json.load(fh)
        choco_feat = [f for f in data["features"]
                          if f["properties"].get("NAME_1") == "choco"]
        if choco_feat:
            with open(dept_out, "w") as fh:
                json.dump({"type": "FeatureCollection", "features": choco_feat}, fh)
            print(f"  -> Saved {dept_out.name} (1 feature)")
        else:
            print("  WARNING: choco not found in GADM L1 data")

    # -- choco 125 municipalities --
    l2_path = BOUNDARIES_DIR / "gadm41_COL_2.json"
    muns_out = BOUNDARIES_DIR / "choco_municipalities_125_GADM41.geojson"
    if not muns_out.exists() and l2_path.exists():
        with open(l2_path) as fh:
            data = json.load(fh)
        ant_muns = [f for f in data["features"]
                    if f["properties"].get("NAME_1") == "choco"]
        print(f"  -> Found {len(ant_muns)} choco municipalities in GADM L2")
...
def download_geoboundaries() -> None:
    ...
    # Extract choco from geoBoundaries ADM1
    adm1_path = BOUNDARIES_DIR / "geoBoundaries_COL_ADM1_departments.geojson"
    adm1_ant_out = BOUNDARIES_DIR / "choco_department_boundary_geoBoundaries.geojson"
    if adm1_path.exists() and not adm1_ant_out.exists():
        with open(adm1_path) as fh:
            data = json.load(fh)
        feat = [f for f in data["features"]
                if f["properties"].get("shapeName") == "choco"]
        if feat:
            with open(adm1_ant_out, "w") as fh:
                json.dump({"type": "FeatureCollection", "features": feat}, fh)
            print(f"  -> Extracted choco from geoBoundaries ADM1")

    # Spatially extract choco municipalities from ADM2
    adm2_path = BOUNDARIES_DIR / "geoBoundaries_COL_ADM2_simplified.geojson"
    adm2_ant_out = BOUNDARIES_DIR / "choco_municipalities_geoBoundaries_simplified.geojson"
    ant_boundary_path = BOUNDARIES_DIR / "choco_department_boundary_GADM41.geojson"

    if (adm2_path.exists() and ant_boundary_path.exists()
            and not adm2_ant_out.exists()):
        print("  -> Spatially extracting choco municipalities from geoBoundaries ADM2...")
        gdf = gpd.read_file(adm2_path)
        choco = gpd.read_file(ant_boundary_path)
        gdf_centroids = gdf.copy()
        import warnings
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            gdf_centroids.geometry = gdf.geometry.centroid
        joined = gpd.sjoin(gdf_centroids, choco[["geometry"]],
                           how="inner", predicate="within")
        result = gdf[gdf.index.isin(joined.index)]
        save_geojson(result, adm2_ant_out, "choco municipalities (geoBoundaries simplified)")


def download_natural_earth() -> None:
    """
    Download Natural Earth 1:10m admin-1 shapefile and extract
    choco + all Colombia departments.
    """
    ...
    ant_out = BOUNDARIES_DIR / "choco_department_naturalearth.geojson"
    ...
    if shp_path.exists():
        ...
        choco = colombia[colombia["name"] == "choco"]
        if not ant_out.exists():
            save_geojson(choco, ant_out, "choco boundary (Natural Earth)")


# ---------------------------------------------------------------------------
# Section 2 – choco Subregions
# ---------------------------------------------------------------------------

def create_choco_subregions() -> None:
    """
    Build the 9 official subregions of choco by dissolving GADM
    municipality polygons. Subregion classification follows Gobernacion
    de choco / DANE official groupings.
    """
    print("\n[5] Creating choco 9 Subregion Boundaries")

    out_path = BOUNDARIES_DIR / "choco_9_subregions.geojson"
    if out_path.exists():
        print(f"  -> Already exists: {out_path.name}")
        return

    muns_path = BOUNDARIES_DIR / "choco_municipalities_125_GADM41.geojson"
    if not muns_path.exists():
        print("  WARNING: GADM municipalities file not found. Run extract_choco_gadm() first.")
        return
    ...
    SUBREGIONS = {
        ...
        "Occidente": [
            ...
            "Santafédechoco", "ElSopetrán", "Uramita",
        ],
        "Magdalena Medio": [
...
def download_hydrobasins() -> None:
    ...
    """
    print("\n[6] HydroBASINS – River Basins for choco")
    print("    Source: https://data.hydrosheds.org")

    ant_path = BOUNDARIES_DIR / "choco_department_boundary_GADM41.geojson"
    if not ant_path.exists():
        print("  WARNING: choco boundary not found. Run earlier steps first.")
        return

    gpd = import_geopandas()
    choco = gpd.read_file(ant_path)
    ...
    for level in [5, 7]:
        ...
        out_name = f"choco_river_basins_HydroSHEDS_L{level}.geojson"
        ...
        if shp_path.exists():
            print(f"  -> Clipping HydroBASINS L{level} to choco...")
            hb = gpd.read_file(shp_path)
            clipped = gpd.sjoin(hb, choco[["geometry"]], how="inner",
                                predicate="intersects")
            clipped = clipped.drop(columns=["index_right"], errors="ignore")
            save_geojson(clipped, out_path,
                         f"choco river basins HydroSHEDS L{level}")
...
def download_osm_data() -> None:
    ...
    After downloading, use osmium or ogr2ogr to clip to choco.

    For a smaller targeted download, use the BBBike custom extract service:
      https://extract.bbbike.org/
    Define the choco bounding box: W=-77.15, S=5.41, E=-73.87, N=8.89
    ...
    print("")
    print("    For choco-only OSM extract (recommended):")
...
def download_flood_hazard_info() -> None:
    """
    Prints information about flood hazard data sources for choco.
    ...
    sources = [
        ...
        {
            "name": "Corchoco – Cuencas (ArcGIS REST)",
            "url": "https://geografico.corchoco.gov.co/arcgis/rest/services/Corchoco_Geoconsulta/MapServer/6",
            "format": "ArcGIS REST / GeoJSON",
            "access": "REST API (connection issues observed)",
            "notes": "Watershed delineation (Otto Pfafstetter NSS3 level) for Corchoco jurisdiction",
        },
        {
        ...
        },
        {
            ...
            "notes": "Search for 'inundaciones', 'eventos', 'emergencias choco'",
        },
        {
...
def main() -> None:
    print("=" * 70)
    print("choco Flood Risk Research – Geographic Boundary Downloader")
    print("=" * 70)
