...
========================================
Nature-quality figures for choco flood risk manuscript.
...
def load_boundary():
    return gpd.read_file(BOUNDARIES_DIR / "choco_department_boundary_GADM41.geojson")


def load_municipalities():
    return gpd.read_file(BOUNDARIES_DIR / "choco_municipalities_125_GADM41.geojson")


def load_subregions():
    return gpd.read_file(BOUNDARIES_DIR / "choco_9_subregions.geojson")
...
def fig01_study_area():
    ...
    choco_p = choco.to_crs(CRS_COLOMBIA)
    ...
    if has_colombia:
        ...
        choco_p.plot(ax=ax, color='#d4a574', edgecolor='#333333', linewidth=0.5)
    else:
        choco_p.plot(ax=ax, color='#d4a574', edgecolor='#333333', linewidth=0.5)
    ...
    choco_p.boundary.plot(ax=ax, color='#333333', linewidth=0.6)
...
def fig02_sar_water_detection():
    ...
    for ax, img, label, title in panels:
        ...
        choco.boundary.plot(ax=ax, color='#444444', linewidth=0.3)
