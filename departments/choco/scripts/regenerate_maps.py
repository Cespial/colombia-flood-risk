...
as PNG arrays, then overlays on proper choco boundaries (WGS84).
...
from utils import (
    load_department_boundary, load_municipalities,
...
def get_choco_region():
    """Get choco geometry from FAO GAUL in GEE."""
    choco = (
        ee.FeatureCollection('FAO/GAUL/2015/level1')
        .filter(ee.Filter.eq('ADM0_NAME', 'Colombia'))
        .filter(ee.Filter.eq('ADM1_NAME', 'choco'))
    )
    return choco.geometry().dissolve()


def get_choco_bbox(region):
    """Get the bounding box of choco as [west, south, east, north]."""
...
def fig02_sar_water_detection():
    ...
    fig.suptitle(
        "Sentinel-1 SAR Flood Detection — Bajo Cauca, choco",
...
def fig03_jrc_water_occurrence():
    """Download real JRC water occurrence and overlay on choco boundary (WGS84)."""
    print("Generating Figure 3: JRC water occurrence (REAL DATA)...")
    set_publication_style()

    region = get_choco_region()
    bbox = get_choco_bbox(region)
    choco_gdf = load_department_boundary("gadm")  # WGS84
    ...
    choco_gdf.boundary.plot(ax=ax, color="black", linewidth=1.2)

    # Set axis limits to choco extent with small padding
    pad = 0.05
    ...
    ax.set_title("JRC Global Surface Water Occurrence, choco", fontsize=10)
    add_north_arrow(ax)
